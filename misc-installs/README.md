...
Strongly suggest to read the [official documentation](https://httpd.apache.org/docs/trunk/install.html#download).

- download from the [stable release](https://httpd.apache.org/download.cgi#apache24)
```
wget https://dlcdn.apache.org/httpd/httpd-2.4.53.tar.gz
tar xzvf httpd-2.4.53.tar.gz && rm httpd-2.4.53.tar.gz
cd httpd-2.4.53
```
...
1. compile & make
```
./configure --prefix=/home/chuqi/local/httpd2 --disable-shared 
make -j > make.tmp         # record the intermediate results of GNU Make
```

Since we want to use static library of libapr & libapr-utils (.a), we should modify the output generated by `libtools`. 

We found that our executable binary `httpd` is generated via:
```
/usr/share/apr-1.0/build/libtool --silent --mode=link x86_64-linux-gnu-gcc  -pthread -o httpd  modules.lo buildmark.o -export-dynamic server/libmain.la modules/core/libmod_so.la modules/http/libmod_http.la server/mpm/event/libevent.la os/unix/libos.la -lpcre /usr/lib/x86_64-linux-gnu/libaprutil-1.la /usr/lib/x86_64-linux-gnu/libapr-1.la 
```
In this link procedure, the shared libraries are specified in file `libapr-1.la` and `libaprutil-1.la`. In order to link those static versions of libraries (.a), we should specify explicitly:

2. re-link the output object

We replace those `.la` files for libAPR by the corresponding `.a` files, to explicitly use the static libraries.
However, in such case, we should also add other dependencies after those static libraries.
```
/usr/share/apr-1.0/build/libtool --silent --mode=link x86_64-linux-gnu-gcc  -pthread -o httpd  modules.o buildmark.o  -export-dynamic server/libmain.la modules/core/libmod_so.la modules/http/libmod_http.la server/mpm/event/libevent.la os/unix/libos.la /usr/lib/x86_64-linux-gnu/libapr-1.a /usr/lib/x86_64-linux-gnu/libaprutil-1.a  -lpcre -luuid -lcrypt -ldl -lexpat
```
...
$ ./sqlite3 -line /path/to/eval.db "select * from users;"
```
...
- download from the [stable release](https://github.com/apple/cups/releases/download/v2.3.0/cups-2.3.0-source.tar.gz)
```
wget https://github.com/apple/cups/releases/download/v2.3.0/cups-2.3.0-source.tar.gz
tar xzvf cups-2.3.0-source.tar.gz
cd cups-2.3.0-source
```

- install
```
./configure --prefix=/home/chuqi/local/cups
sudo make && make install
```
